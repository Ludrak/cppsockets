.PHONY: clean fclean re all os check_sources check_headers client_build server_build
...
#   Main rule
all: display_os client_build server_build
	@ echo "$(PREFIX_INFO) all done."

# Server rule, for making server only
server_build: NAME = $(NAME_SERVER)
server_build: SRC_FILES = $(SERVER_SRC_FILES)
server_build: SRC_DIR = $(SERVER_SRC_DIR)
server_build: display_os init_lib check_headers check_sources $(NAME_SERVER)
	@ echo "$(PREFIX_INFO) All done for server"

# Client rule, for making client only
client_build: NAME = $(NAME_CLIENT)
client_build: SRC_FILES = $(CLIENT_SRC_FILES)
client_build: SRC_DIR = $(CLIENT_SRC_DIR)
client_build: display_os init_lib check_headers check_sources $(NAME_CLIENT)
